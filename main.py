...
class PaymentProviderFetcher():
    ...
    def get_product(self, website: str):
        ...
        if sitemap_1 != -1:
            ...
            sitemap = re.search("(?P<url>https?://[^\s]+)", sitemap).group("url")

            _sitemap = urlparse(sitemap)
            _sitemap = _sitemap._replace(netloc=website.replace("https://", "").replace("http://", "").replace("/", ""))
    ...
    def get_checkout_btn(self):
        ...
        if not btn:
            btn = self.get_element_by_xpath(
                "//*[translate(normalize-space(), 'fortsæt til kassen', 'FORTSÆT TIL KASSEN')='FORTSÆT TIL KASSEN']/*")
            x_path = "//*[translate(normalize-space(), 'fortsæt til kassen', 'FORTSÆT TIL KASSEN')='FORTSÆT TIL KASSEN']/*"
